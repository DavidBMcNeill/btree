...
public class BTreeFile {
	...
	public BTreeFile() throws IOException {
		...
		this.metadata = new BTreeMetadata(ArgsGenerate.degree, BTreeNode.SIZE, ArgsGenerate.sequenceLength);
	...
	private void createAccessFile(File f) throws IOException {
		// "rwd" tells the reader to update the files's content on disk for
		// every update.
		// this is more efficient and guarantees all modifications have been
		// made.
	...
	private File generateBtreeFile() throws IOException {
		...
		// System.out.printf("ArgsGenerate.geneBankFile.getAbsolutePath():
		...
		String name = String.format("%s.btree.data.%s.%s", ArgsGenerate.geneBankFile.getAbsolutePath(),
				metadata.sequenceLength, metadata.degree);
	...
	/**
	 * Reads the BTree's metadata in the binary btree file, returns the data as
	 * a BTreeMetadata class.
	 */
	public BTreeMetadata readTreeMetadata() {
		...
		try {
			...
			return new BTreeMetadata(file.readInt(), // degree
	...
	public long write(BTreeNode node) {
		...
		try {
			...
			// start after the btree metadata, then go "index" node "size"s
			// over.
			long spot = BTreeMetadata.SIZE + (node.getId() * BTreeNode.SIZE);

			if (spot == BTreeMetadata.SIZE + (2 * BTreeNode.SIZE)) {
				System.out.println("stop overwriting!");
			}
