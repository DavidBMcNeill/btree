...
/**
 * Cache implementing a doubly-linked list of BTreeNodes to improve BTree
 * runtime. Cache size specified by command-line argument from GeneBankSearch
 * call, and GeneBankCreateBTree call.
 * 
...
public class Cache {
	...
	private IUDoubleLinkedList cache;
	int sizeLimit;
	BTreeFile file;
	...
	public Cache(int size, BTreeFile file) throws IOException {
		sizeLimit = size;
		this.file = file;
		this.cache = new IUDoubleLinkedList();
	}

	/*
	 * 1) if cache hit, +move hit data item to top of cache
	 */
	...
	public BTreeNode getNode(int nodeIndex) {
		...
		if (nodeIndex > -1 && nodeIndex < cache.size()) {
			getNode = (BTreeNode) cache.get(nodeIndex);
		}
		return getNode;
	}
	...
	public BTreeNode findKey(long key) {
		...
		for (int i = 0; i < sizeLimit-1; i++) {
			...
			for (int j = 0; j < node.getNumObjects(); j++) {
				TreeObject object = node.peekObject(j);
...
//		clearCache();
	...
	public void removeFirst() {
		BTreeNode first = (BTreeNode) cache.remove(0); // cache.size() - 1
		file.write(first);
	}
	...
	public void bumpUp(BTreeNode element) {
		...
		BTreeNode storage = (BTreeNode) cache.remove(swapIndex);
		cache.add(0, storage);
	}
